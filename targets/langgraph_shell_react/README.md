...
Replay and fork lifecycle tasks now default to the durable `disk` checkpoint backend. That backend persists checkpoint state under `langgraph-checkpoints/` inside the SyncFuzz workspace and describes the resulting files in `langgraph-checkpointer.json`. The V2.4 recovery observer can additionally read `/proc/net/unix` and holder FDs when its controller explicitly joins a retained source runtime; it does not connect to the service.
...
For native-checkpoint recovery plumbing, `--checkpoint-id` selects one exact durable LangGraph checkpoint in a fresh resume process. It is mutually exclusive with `--checkpoint-index`, `--checkpoint-coordinate-file`, and `--checkpoint-selector`; a recovery executor must clone the original disk checkpointer store before using that ID. `--checkpoint-coordinate-file` remains available for diagnostic equivalence checks, but it is not the recovery selection mechanism because a live model may produce a different history on re-execution. `--passive-fork-observe` performs no follow-up agent turn; it only rehydrates the selected state in a fresh Python process and records the fixed observation. It requires `--passive-unix-socket-path branch-listener.sock`, which writes `langgraph-recovery-observation.json` with pre/post `lstat` metadata (device, inode, mode and existence) for that workspace-contained endpoint. It never connects to, creates, replaces, or unlinks the socket, so this is a fixed OS-level passive observation, not a protocol-level client action.
