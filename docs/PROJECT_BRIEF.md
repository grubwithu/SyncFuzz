...
Phase 5B 当前已经先打通了这一缺口所需的执行底座：real-target matrix candidate 中的完整 Scenario IR 会进入真实 target 环境，控制 prompt、replay/fork、checkpoint selector、durable backend、process mode、follow-up、oracle、compliance、contract 与 signature，并原样记录到 `target-task.json`。matrix 已能自动把 split-process checkpoint candidate 派生为 `phase-shift-single-process` candidate，也已经形成第一组受控 primitive substitutions：从 PATH fork seed 自动得到 env-export 与 function-define 两个 candidate，保留 fork lifecycle，但替换 plant、checkpoint selector 与 activation/oracle binding。第一条 activation substitution 也已落地：从 Unix-listener fork seed 派生 trusted-action candidate，保留 listener plant，但把被动 reachability 替换为固定 successor-branch policy 与本地 consequence artifact。它们是研究者未手写成独立 task 的新组合，不过当前 family 仍然很小。scheduler 同时按 activation progress 保留进入更深阶段的 candidate，并把 lifecycle、plant、activation stall 映射到定向 repair variant。下一步是继续扩展 primitive / activation substitution family，再推进跨 operation lifecycle splice 和跨 seed 组合；在完成前，项目仍应把当前能力表述为 feedback-guided scenario exploration，而不是完整自动场景合成。
