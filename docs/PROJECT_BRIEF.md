...
Minimization 也开始形成执行闭环。`target minimize` 默认仍只抽取结构化 plan；显式 `--execute` 时会从原始 `target-task.json` 恢复真实 target command，逐行尝试缩减 prompt，然后逐项清除 execution plan 中的 process mode、checkpoint backend、checkpoint selector、fork follow-up 与 replay，并要求 oracle、attribution、signature、task compliance 与 contract interpretation 保持不变。component、primitive 与 activation-command reduction 仍待实现，但 minimizer 已经可以真实收紧 prompt 和 lifecycle plan，而不只是生成清单。
