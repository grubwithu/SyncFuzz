...
Phase 5B 当前已经先打通了这一缺口所需的执行底座：real-target matrix candidate 中的完整 Scenario IR 会进入真实 target 环境，控制 prompt、replay/fork、checkpoint selector、durable backend、process mode、follow-up、oracle、compliance、contract 与 signature，并原样记录到 `target-task.json`。matrix 已能自动把 split-process checkpoint candidate 派生为 `phase-shift-single-process` candidate，也已经形成第一组受控 primitive substitutions。除了原先的 PATH fork seed 会自动得到 env-export 与 function-define 两个 fork candidate 之外，PATH same-run seed 现在也会自动得到 `persistent-shell-poisoning/primitive-shell-env-export`、`persistent-shell-poisoning/primitive-shell-function-define`、`persistent-shell-poisoning/primitive-shell-cwd-change` 和 `persistent-shell-poisoning/primitive-shell-umask-set` 这组 portable candidate，保留 `run -> continue` lifecycle，并通过通用 `env-residue` / `function-residue` / `cwd-residue` / `umask-residue` oracle 与 compliance dispatch 同时覆盖 LangGraph 和 MAF；LangGraph 还会用对应的 same-run contract rule 分别解释这些 execution-context surface，而不是回退到 PATH baseline 解释。PATH replay seed 现在也会继续派生 `persistent-shell-poisoning-replay/primitive-shell-env-export` 与 `persistent-shell-poisoning-replay/primitive-shell-function-define`，保留 `checkpoint -> replay` lifecycle，并显式区分 direct replay residue、replay-side reexecution 与 final-call reconstruction。activation substitution 也开始进入 portable same-run family：`unix-listener-residue/activation-trusted-action` 会保留同一 run 内的 listener plant，但把被动 socket witness 替换为固定 trusted policy，并用同一 generated Scenario IR 同时覆盖 LangGraph 和 MAF。与此同时，Unix-listener fork seed 与 inherited-FD fork seed 也都能派生 trusted-action candidate，分别覆盖 communication 和 resource-access capability，两者都保留原 fork lifecycle，但把被动 witness 替换为固定 successor-branch policy 与本地 consequence artifact。第一条生成式 lifecycle splice 也已经出现：Unix-listener fork seed 现在还能派生 replay splice candidate，保留同一 plant，但把 lifecycle 改成 `checkpoint -> replay`，并显式区分 direct residue 与 replay-side relaunch。它们是研究者未手写成独立 task 的新组合，不过当前 family 仍然很小。scheduler 同时按 activation progress 保留进入更深阶段的 candidate，并把 lifecycle、plant、activation stall 映射到定向 repair variant。下一步是继续扩展 primitive / activation / lifecycle-splice family，再推进跨 seed 组合；在完成前，项目仍应把当前能力表述为 feedback-guided scenario exploration，而不是完整自动场景合成。

Minimization 也开始形成执行闭环。`target minimize` 默认仍只抽取结构化 plan；显式 `--execute` 时会从原始 `target-task.json` 恢复真实 target command，逐行尝试缩减 prompt，再尝试删除 optional `setup` / `fault` Scenario IR component，并在 `semantic` / `impact` fidelity 下尝试清除可缩减的 plant primitive metadata，还可以对 multi-line fork activation message 做行级缩减，然后逐项清除 execution plan 中的 process mode、checkpoint backend、checkpoint selector、fork follow-up 与 replay。默认 `exact` fidelity 要求 oracle、attribution、signature、task compliance 与 contract interpretation 保持不变；`semantic` 和 `impact` fidelity 则分别支持更宽松的语义保真与影响保真。command-level primitive、non-fork activation-command、lifecycle / activation / oracle component 与 cross-seed reduction 仍待实现，但 minimizer 已经可以真实收紧 prompt、optional IR component、plant metadata、fork activation message 和 lifecycle plan，而不只是生成清单。
