...
Phase 5B 当前已经先打通了这一缺口所需的执行底座：real-target matrix candidate 中的 Scenario IR `execution_plan` 会进入真实 target 环境，控制 replay/fork、checkpoint selector、durable backend、process mode 与 follow-up，并原样记录到 `target-task.json`。在此基础上，matrix 已能自动把 split-process checkpoint candidate 派生为 `phase-shift-single-process` candidate；这是第一条由 mutator 生成、且真正改变 runtime execution boundary 的 real-target candidate。scheduler 同时开始按 activation progress 保留进入更深阶段的 candidate，并把 lifecycle、plant、activation stall 映射到定向 repair variant。下一步仍然是 primitive substitution、跨 operation lifecycle splice 和跨 seed 组合；在完成前，项目仍应把当前能力表述为 feedback-guided scenario exploration，而不是完整自动场景合成。
