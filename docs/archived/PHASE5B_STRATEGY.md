...
2. 在首条 process-mode phase shift 基础上，继续实现 primitive substitution、跨 operation lifecycle splice 和更多 phase shift，而不是只派生 prompt variant。
...
`MAF-1` 现在已经完成第一轮 same-run residue 对齐，`MAF-2` 也已经有了第一条 `AgentSession` serialization / restore smoke path，`MAF-3` 已经完成最小 Workflow checkpoint continuity smoke path。因此这一轮后续的目标不再是补更多普通 workspace object，而是把 Workflow checkpoint 从 smoke path 推向更有研究价值的恢复语义：

> `executor -> superstep -> checkpoint -> resume / rehydrate`

并围绕它测试 external effect replay、partial commit commit、pending approval replay、same-instance resume 和 new-instance rehydrate 的状态分叉。
...
当前已经完成 minimization 的前三层：confirmed target result 会附带 `syncfuzz.target-minimization-plan.v1`；`target minimize` 会把 suite / matrix 结果抽成 `syncfuzz.target-minimization-batch.v1`；显式 `target minimize --execute` 会真正执行 prompt-line 与 execution-plan reduction 并自动复验。下一步是让 runner 消费 component / primitive / activation-command / mutation-axis step，并把 reduction 结果直接反馈给 corpus 与 campaign。
...
- MAF-2 至少有一条 session restore smoke path 能进入 suite / matrix / campaign。
...
2. `MAF-2`
   官方 shell sample 加 session restore，验证 logical session 与 OS state 是否分叉。当前第一步是 `maf-session-continuity`，通过 serialized `AgentSession` 和新 runtime object 建立可观测恢复边界。
3. `MAF-3`
   最小 Workflow 加 `CheckpointStorage`，正式测试 superstep checkpoint、resume / rehydrate、pending request / response 与 external effect replay。当前已经有 `maf-workflow-checkpoint-continuity`、`maf-workflow-external-effect-replay`、`maf-workflow-http-effect-replay`、`maf-workflow-resource-replay`、`maf-workflow-authority-token-replay`、`maf-workflow-partial-commit-replay`、`maf-workflow-approval-pending-replay` 和 `maf-workflow-rehydrate-divergence`，已把 official Workflow file checkpoint restore 接入 target oracle / task compliance；HTTP-backed tasks 现在可以通过 `MAF_WORKFLOW_EFFECT_SERVICE_URL` 绑定独立 mock service process，下一步是把 authority/token 推进到更真实的 SSH / OAuth / CI token 边界。
