...
3. `target compare` 已将 checkpoint-bound root-cause hypothesis 收紧为 contract-calibrated 条件：同一 profile/rule 的 task-compliant pair 必须呈现 target violation / control consistency；`target runtime-pair` 已能在隔离 workspace 中执行显式 control/target command 并立即生成 pair differential，`target pair-campaign` 既可从预记录 manifest 读取，也可直接读取多个 runtime-pair artifact，将这些 run 按 fresh-runtime、branch-cleanup、namespace-restore 等 control kind 与 Query root / violation signature / mutation axis 汇总 deterministic counterfactual label；`target calibration-summary` 已汇总 campaign 的 calibration coverage、unresolved reason 与 provenance，并仅在独立 candidate-level review manifest 存在时量化 reviewed hypothesis precision；下一步用真实模型 runtime 的控制命令填入这些 strata；
