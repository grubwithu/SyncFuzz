...
2026-07-25 的增量实现完成了第二个 LangGraph retained-resource family 的代码闭环：`handle.workspace-file.survival` 通过 exact `handle/open` evidence 绑定 materialization-head 的 regular workspace file，冻结 source file 的 device/inode/mode，并在 fresh recovery container 中只用 `lstat` identity probe 和 read-only bind mount 判断 residual。该 family 仍只支持 full passive mode，尚未执行真实 generated candidate 或纳入 listener fidelity batch；下一步是用外部 StateFuzz candidate 跑一次真实 profile、binding、StateSeed promotion 与 recovery-set，确认它能在不引入内容 Oracle 的情况下形成可审计证据。
