...
- `MAF-3` 已经有第一批最小 Workflow checkpoint smoke path：`maf-workflow-checkpoint-continuity` 使用官方 `WorkflowBuilder` / `Executor` / `FileCheckpointStorage`，在 checkpoint 后重建 workflow object 并从 checkpoint restore；`maf-workflow-external-effect-replay` 把 post-restore activation 换成 non-idempotent ledger append，用于观测一次 logical operation 是否产生重复 external effect。后续再扩大到真实外部服务、parallel partial commit 和 pending approval replay。
...
并围绕它测试 external effect replay、parallel partial commit、pending approval replay、same-instance resume 和 new-instance rehydrate 的状态分叉。
...
3. `MAF-3`
   最小 Workflow 加 `CheckpointStorage`，正式测试 superstep checkpoint、resume / rehydrate、pending request / response 与 external effect replay。当前已经有 `maf-workflow-checkpoint-continuity` 和 `maf-workflow-external-effect-replay`，已把 official Workflow file checkpoint restore 接入 target oracle / task compliance；下一步是从本地 ledger 级 replay 扩展到真实外部服务与 parallel partial commit。
