...
当前已经完成 minimization 的前两层：confirmed target result 会附带 `syncfuzz.target-minimization-plan.v1`，列出 prompt reduction、component deletion、primitive minimization、lifecycle tightening、activation minimization、oracle preservation、mutation-axis check 和 replay check；`target minimize` 会把 suite / matrix 结果抽成 `syncfuzz.target-minimization-batch.v1`。下一步是让 runner 消费这个 batch，真正执行删减候选并自动复验。
