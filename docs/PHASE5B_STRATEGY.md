...
`MAF-1` 现在已经完成第一轮 same-run residue 对齐，`MAF-2` 也已经有了第一条 `AgentSession` serialization / restore smoke path。因此这一轮后续的目标不再是补更多普通 workspace object，而是继续把恢复模型往 Workflow checkpoint 推进：
