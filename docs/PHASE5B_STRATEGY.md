...
- `target scenarios` 已提供第一版 Scenario IR view：state surface、lifecycle edge、setup / plant / activation / oracle。
...
3. 基于 `unix-listener-residue-fork` 派生 trusted-client activation 和 response poisoning 场景。
...
这两项继续留在路线里，但定位是快速补齐 shell execution context family，而不是下一阶段的核心成果。
