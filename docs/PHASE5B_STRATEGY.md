...
oracle、contract interpretation 或 root-cause verdict。

该 proposal boundary 现已由 `target contract-propose` 实现：调用者显式选择
target、内建 task context、`source_root` 与相对 source file allowlist。使用
`--provider openai-compatible` 时，Go 内置 client 读取 `OPENAI_API_KEY`、可选
`OPENAI_BASE_URL` 与显式 `CONTRACT_PROPOSAL_MODEL`，发送一次受限 Chat
Completions request；system prompt 固化完整 candidate JSON shape、必填字段、枚举和
精确 line-span 字段名。该 provider mode 必须显式选择，且 run artifact 只记录
`generator_kind`、model 与 prompt version，不记录 key、endpoint、prompt text 或
provider error body。
随后 validator 除 span/quote/path 校验外，还要求 citation 位于本次 request 的
allowlist，并固定 `automatic_profile_adoption=disabled`，不会改写 profile 或影响
oracle。

`--generator-command` 仍保留为 caller-supplied、未 sandbox 的外部 generator
interface：SyncFuzz 通过 request/output 环境变量交给命令，再回验其输出 citation；
对应 run 仅保存 command hash。`examples/target-contract-proposal-generator.example.sh`
只是 deterministic I/O fixture，不是 LLM。两种模式的输出都不会被加载为 profile
或 oracle input。
...
下列结果是当前代码与模型配置下的实验快照，不是统计显著性或因果结论。生成 artifact 不入 Git；这里记录完整路径、预算与汇总，便于后续复查。

**Feedback policy comparison.** 在 `langgraph-shell-react` 的 74-candidate universe 中，固定 `baseline` prompt、`4` rounds × `8` candidates（每策略 32 runs）。早期 block 1–2 使用旧 scheduler，未计入下面的冻结汇总；block 3–5 使用 penalty-aware feedback：`task-noncompliant` / `execution-not-reached` 不触发 prompt repair，且会抑制同一 executable scenario 的未执行 sibling。

| policy | blocks / artifacts | total runs | confirmed | residue observed | activation reached | scenario avg. | violation-signature avg. | transition avg. |
|---|---|---:|---:|---:|---:|---:|---:|---:|
| fixed | `block-3..5/fixed/target-campaign-{1784711903668083519,1784714206919725414,1784716103411263596}` | 96 | 44 | 31 | 31 | 17.0 | 6.0 | 17.0 |
| random | `block-3..5/random/target-campaign-{1784711903652685414,1784714206980353532,1784716103440027752}` | 96 | 41 | 24 | 28 | 22.0 | 9.3 | 21.0 |
| feedback | `block-3..5/feedback/target-campaign-{1784711903636971552,1784714206916092069,1784716103410919098}` | 96 | **46** | **36** | **44** | **25.7** | **10.3** | **23.7** |

在这三个 blocked repeat 中，feedback 在 activation、residue observation 与三项 coverage 指标上最高；fixed 的 `task-noncompliant` 较少（35 vs feedback 38），random 的 `execution-not-reached` 较少（13 vs feedback 14）。因此当前可陈述的是“该配置下有初步正向信号”，不能写成普适优越性，也不能用它证明 finding 的根因。

**Semantic-mutator ablation.** 使用同一七个 seed family、`baseline` prompt、penalty-aware feedback 与 `4` rounds × `4` candidates（每 scope 16 runs）。`seed-only` 只含 base scenario；`seed-plus-prompts` 增加同一 scenario 的 prompt variant；`all` 还含完整 semantic derivation。block 1–2 artifact 为：

| scope | artifacts | total runs | confirmed | residue observed | activation reached | task noncompliant | violation signatures |
|---|---|---:|---:|---:|---:|---:|---:|
| seed-only | `block-{1,2}/seed-only/target-campaign-{1784719267153549863,1784720837467102307}` | 32 | **18** | 11 | 11 | 17 | **18** |
| seed-plus-prompts | `block-{1,2}/seed-plus-prompts/target-campaign-{1784719278449436452,1784720847609353395}` | 32 | **18** | **12** | **12** | **15** | 17 |
| all | `block-{1,2}/all/target-campaign-{1784719281389400101,1784720856794376124}` | 32 | 16 | 11 | 9 | **15** | 15 |

`all` 确实执行了 activation substitution、phase shift、lifecycle splice 与 primitive substitution，并有 IPC trusted-client / response-cache 与 process phase-shift 的 activated artifacts；但在这两个 block 中，它没有取得固定预算收益。当前冻结结论是：**semantic mutator 的可执行性已经验证，效率收益尚未验证。** 后续只有在按 mutation family 校准 noncompliance、保持独立 prompt-only control 并增加 repeat 后，才能重新宣称 mutator contribution；在此之前该负向消融必须保留。
...
- feedback guidance 已从二值 activation reached 扩展为 execution、task compliance、lifecycle、plant、activation、reached 多阶段进度；candidate ranking 与 coverage gain 会消费全部进度，但 prompt repair 只消费可修复的 lifecycle、plant 与 activation 缺口。`task-noncompliant` 与 `execution-not-reached` 是强惩罚而不是 repair trigger，且 repair scope 限于同一个 executable scenario，避免一个 mutation 的 prompt drift 消耗另一 mutation 的预算。frontier 还能把修复意图标记为 `lifecycle-repair`、`state-plant-repair` 或 `activation-repair`。
- 为把 mutator 的贡献与 seed corpus、prompt variant 区分开，target matrix / suite / campaign 现在固化 `candidate_scope`：`seed-only` 仅保留 base seed scenario，`seed-plus-prompts` 保留同一 scenario 的 prompt variant 但移除 semantic derivation，`all` 保留完整 semantic-mutator universe。三者使用同一 selected seed family 与预算即可构成受控消融，不能把 scope 变化后的覆盖差异误报为单纯 scheduler 收益。
