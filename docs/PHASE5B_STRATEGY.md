...
3. `MAF-3`
   最小 Workflow 加 `CheckpointStorage`，正式测试 superstep checkpoint、resume / rehydrate、pending request / response 与 external effect replay。当前已经有 `maf-workflow-checkpoint-continuity`、`maf-workflow-external-effect-replay`、`maf-workflow-http-effect-replay`、`maf-workflow-partial-commit-replay`、`maf-workflow-approval-pending-replay` 和 `maf-workflow-rehydrate-divergence`，已把 official Workflow file checkpoint restore 接入 target oracle / task compliance；下一步是从 local HTTP service boundary 扩展到多进程真实外部服务。
