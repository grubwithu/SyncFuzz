...
该 generator boundary 现已由 `target contract-propose` 实现：调用者显式选择
target、内建 task context、`source_root` 与相对 source file allowlist，再提供自己
的 `--generator-command`。SyncFuzz 把这些内容和 SHA-256 固化为
`syncfuzz.target-contract-proposal-request.v1`，通过 request/output 环境变量交给
命令；命令只能写回既有 candidate-set schema。随后 validator 除 span/quote/path
校验外，还要求 citation 位于本次 request 的 allowlist。run artifact 仅保存
generator command 的 hash，避免持久化可能包含 credential 的命令文本；固定
`automatic_profile_adoption=disabled`，不会自动调用 provider、改写 profile 或影响
oracle。`examples/target-contract-proposal-generator.example.sh` 只是 deterministic
I/O fixture，不是 LLM。`generator-command` 是调用者显式提供且未 sandbox 的本地
命令；SyncFuzz 的 proposal pipeline 不会把它的输出加载为 profile 或 oracle input。
