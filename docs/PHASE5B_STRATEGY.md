...
- `MAF-3` 已经有第一批最小 Workflow checkpoint smoke path：`maf-workflow-checkpoint-continuity` 使用官方 `WorkflowBuilder` / `Executor` / `FileCheckpointStorage`，在 checkpoint 后重建 workflow object 并从 checkpoint restore；`maf-workflow-external-effect-replay` 把 post-restore activation 换成 non-idempotent ledger append；`maf-workflow-partial-commit-replay` 则在下游失败后验证 partially committed external effect 是否会被 restore 后再次执行。后续再扩大到真实外部服务和 pending approval replay。
...
3. `MAF-3`
   最小 Workflow 加 `CheckpointStorage`，正式测试 superstep checkpoint、resume / rehydrate、pending request / response 与 external effect replay。当前已经有 `maf-workflow-checkpoint-continuity`、`maf-workflow-external-effect-replay` 和 `maf-workflow-partial-commit-replay`，已把 official Workflow file checkpoint restore 接入 target oracle / task compliance；下一步是从本地 ledger 级 replay 扩展到真实外部服务与 pending approval replay。
