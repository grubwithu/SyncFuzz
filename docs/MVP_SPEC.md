...
> **路线状态（2026-07-26）**：本文件描述已完成的 deterministic MVP 与 V2 artifact contract。当前研究路线见 [RESEARCH_PLAN.md](RESEARCH_PLAN.md)，详细 V3 设计见 [RECOVERY_HAZARD_FUZZING.md](RECOVERY_HAZARD_FUZZING.md)。尤其不再将当前 mutation matrix 视为自动发现新 Query 的机制。V2.1a 已新增离线 profiling evidence contract；V2.2 的真实 eBPF/container collector 已完成 process、resource、FD identity 与 Unix-socket closure calibration。

> **V3 边界**：本文中的 `ResourceGraph` / resource dependency 是 profile 或 probe 实际观测到的图，不是由 Fuzzer 生成并 materialize 的 `EnvironmentProgram E`。本文的 `RecoveryRelationReport` 只描述静态 A/O relation；它没有 target recovery-time typed resolve/use `U'` evidence，不能作为 `RecoveryHazardReport` 或漏洞结论。当前 generic continuation `K` 仍是无偏行为刺激，不是独立的 resource-specific `RecoveryUsePlan`；后者目前只在 local fixture calibration 中实现。
...
`synthesis statefuzz-relation-batch-report` is a separate evidence aggregation
stage for accepted roots. It reads only the existing relation reports and their
seed/profile/recovery artifacts, verifies that the three controls share the
same passive observation scope, and counts canonical before/after/head relation
vectors, complete control sets, head consistency, causal-evidence status,
contract-status metadata, and runtime image IDs. It preserves accepted roots
with missing or malformed relation artifacts as `invalid-relation-artifact`
denominator entries. It does not inspect task wording or retained-file content,
and it does not turn a relation into an Oracle or contract verdict.
