...
type VerificationEntryResult struct {
	...
	ActualSignature   core.MismatchSignature `json:"actual_signature,omitempty"`
...
func VerifyCorpus(ctx context.Context, opts VerifyOptions) (*VerificationResult, error) {
	...
	for _, entry := range entries {
		...
		item.AdapterID = core.FirstNonEmpty(replay.AdapterID, item.AdapterID)
	...
	if err := core.WriteJSON(filepath.Join(verifyDir, "verification-result.json"), result); err != nil {
		return nil, err
	}
	return result, nil
}
