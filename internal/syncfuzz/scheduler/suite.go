...
type SuiteCaseResult struct {
	...
	Signature          core.MismatchSignature `json:"signature,omitempty"`
...
func RunSuite(ctx context.Context, opts SuiteOptions) (*SuiteResult, error) {
	...
	selected := opts.Cases
	if len(selected) == 0 {
		selected = core.CaseNames()
	}
	if err := core.ValidateCaseNames(selected); err != nil {
		return nil, err
	}
	...
	if matrix != nil {
		if err := core.WriteJSON(filepath.Join(suiteDir, scheduleMatrixArtifact), matrix); err != nil {
			return nil, err
		}
	}
	...
	if err := core.WriteJSON(filepath.Join(suiteDir, "suite-result.json"), result); err != nil {
		return nil, err
	}
	if err := core.WriteJSON(filepath.Join(suiteDir, "interesting.json"), result.Discoveries); err != nil {
		return nil, err
	}
	if matrix != nil {
		...
		if err := core.WriteJSON(filepath.Join(suiteDir, matrixResultArtifact), matrixResult); err != nil {
			return nil, err
		}
	}
	return result, nil
...
func runSuiteItem(ctx context.Context, opts SuiteOptions, suiteDir string, item *SuiteCaseResult, feedback *suiteFeedback, result *SuiteResult) {
	...
	if opts.Differential {
		pairResult, err := RunPair(ctx, PairOptions{
			...
			TimingProfileID: core.FirstNonEmpty(item.TimingProfileID, opts.TimingProfileID),
		})
		if err != nil {
...
func applyRunResult(item *SuiteCaseResult, runResult *core.RunResult) {
	item.RunID = runResult.RunID
	item.FaultRunID = runResult.RunID
	item.FaultPlanID = runResult.FaultPlanID
	item.PrimitiveID = core.FirstNonEmpty(item.PrimitiveID, runResult.PrimitiveID)
...
func applyPairResult(item *SuiteCaseResult, pairResult *PairResult) {
	...
	item.PrimitiveID = core.FirstNonEmpty(item.PrimitiveID, pairResult.PrimitiveID)
