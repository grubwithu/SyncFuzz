...
func TestVerifyCorpusReportsReproductionAndSignatureDrift(t *testing.T) {
	...
	if result.OutcomeSummaries[0].Category != corpus.ReplayOutcomeReproduced || result.OutcomeSummaries[0].TotalEntries != 1 {
		t.Fatalf("unexpected reproduced outcome summary: %#v", result.OutcomeSummaries[0])
	}
	if result.OutcomeSummaries[1].Category != corpus.ReplayOutcomeSignatureDrift || result.OutcomeSummaries[1].TotalEntries != 1 {
