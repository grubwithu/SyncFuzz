...
func BuildScheduleMatrix(opts MatrixOptions) (*ScheduleMatrix, error) {
	...
	if len(selectedCases) == 0 {
		selectedCases = core.CaseNames()
	}
	if err := core.ValidateCaseNames(selectedCases); err != nil {
		return nil, err
	}
	...
	for _, caseName := range selectedCases {
		faultPlan, err := core.ResolveFaultPlan(caseName, "")
		if err != nil {
			return nil, err
		}
		for _, primitive := range core.PrimitivesForCase(caseName, opts.IncludePlanned) {
			for _, timing := range timingProfiles {
				candidates = append(candidates, ScheduleCandidate{
					...
					StateLayers:     core.UniqueStrings(append(append([]string{}, faultPlan.StateLayers...), primitive.StateLayers...)),
...
func resolveMatrixTimingProfiles(profileIDs []string) ([]core.FaultTiming, error) {
	if len(profileIDs) == 0 {
		return core.TimingProfiles(), nil
	}
	var out []core.FaultTiming
	...
	for _, id := range profileIDs {
		...
		profile, err := core.ResolveTimingProfile(id, 0)
		if err != nil {
			return nil, err
		}
...
func timingProfileIDs(profiles []core.FaultTiming) []string {
