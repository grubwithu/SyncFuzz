...
func RunTargetCampaign(ctx context.Context, opts TargetCampaignOptions) (*TargetCampaignResult, error) {
	...
	result := &TargetCampaignResult{
		...
		PromptProfiles:   append([]string{}, targetPromptProfileSelection(opts.PromptProfileID, opts.PromptProfileIDs)...),
	...
	if err := writeJSON(filepath.Join(campaignDir, targetCampaignResultArtifact), result); err != nil {
		return nil, err
	}
	return result, nil
}
