...
func startPersistentShell(ctx context.Context, dir string) (core.PersistentShell, error) {
	cmd := exec.CommandContext(ctx, "bash", "--noprofile", "--norc")
	cmd.Dir = dir
	return startPersistentShellCommand(ctx, cmd)
}

func startPersistentShellCommand(ctx context.Context, cmd *exec.Cmd) (core.PersistentShell, error) {
...
func (s *persistentShell) Probe(ctx context.Context) (core.ShellState, error) {
	...
	if err != nil {
		return core.ShellState{}, err
	}

	state := core.ShellState{Raw: lines, Aliases: []string{}}
