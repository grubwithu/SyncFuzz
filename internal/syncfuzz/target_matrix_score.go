...
type TargetCandidateSummary struct {
	...
	OracleStatuses          []TargetOracleStatus                 `json:"oracle_statuses,omitempty"`
	ComplianceStatuses      []TargetTaskComplianceStatus         `json:"compliance_statuses,omitempty"`
	ContractStatuses        []TargetContractInterpretationStatus `json:"contract_statuses,omitempty"`
}

type targetCandidateAccumulator struct {
	summary            TargetCandidateSummary
	attributions       map[string]struct{}
	oracleStatuses     map[TargetOracleStatus]struct{}
	complianceStatuses map[TargetTaskComplianceStatus]struct{}
	contractStatuses   map[TargetContractInterpretationStatus]struct{}
}
	...
	for _, result := range results {
		...
		if accumulator == nil {
			accumulator = &targetCandidateAccumulator{
				...
				oracleStatuses:     make(map[TargetOracleStatus]struct{}),
				complianceStatuses: make(map[TargetTaskComplianceStatus]struct{}),
				contractStatuses:   make(map[TargetContractInterpretationStatus]struct{}),
			}
...
func (a *targetCandidateAccumulator) observe(result TargetSuiteRunResult) {
	...
	contractStatus := targetContractInterpretationStatusValue(result.ContractInterpretation)
...
func sortedTargetOracleStatuses(values map[TargetOracleStatus]struct{}) []TargetOracleStatus {
	out := make([]TargetOracleStatus, 0, len(values))
...
func sortedTargetComplianceStatuses(values map[TargetTaskComplianceStatus]struct{}) []TargetTaskComplianceStatus {
	out := make([]TargetTaskComplianceStatus, 0, len(values))
	for value := range values {
		out = append(out, value)
	}
	sort.Slice(out, func(i, j int) bool {
...
func sortedTargetContractStatuses(values map[TargetContractInterpretationStatus]struct{}) []TargetContractInterpretationStatus {
	out := make([]TargetContractInterpretationStatus, 0, len(values))
	for value := range values {
		out = append(out, value)
	}
	sort.Slice(out, func(i, j int) bool {
...
func targetOracleStatusOrder(status TargetOracleStatus) int {
	switch status {
	case targetOracleStatusConfirmed:
		return 0
	case targetOracleStatusInconclusive:
