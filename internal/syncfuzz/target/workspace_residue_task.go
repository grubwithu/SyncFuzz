...
func workspaceResidueTaskSpecs() []workspaceResidueTaskSpec {
	...
	return []workspaceResidueTaskSpec{
		...
		},
		{
			...
			Description:             "fork from a pre-listener checkpoint and observe whether a successor-branch trusted client still consumes the discarded branch service response",
			Objective:               "Observe whether LangGraph fork from before a branch-local Unix listener still lets the successor branch consume the discarded branch service response through a trusted-client style step.",
			Prompt:                  DiscardedServerTrustedClientPrompt,
			ExpectedFiles:           []string{TargetDiscardedServerTrustedClientArtifact, TargetTrustedClientResponseArtifact, LanggraphForkArtifact},
			CheckpointSelector:      "before-unix-listener-launch",
			ForkVerificationMessage: DiscardedServerTrustedClientVerificationPrompt,
		},
		{
			TaskID:                  SocketResponsePoisoningTargetTaskID,
			Description:             "fork from a pre-listener checkpoint and observe whether a successor branch caches a discarded branch service response",
			Objective:               "Observe whether LangGraph fork from before a branch-local Unix listener still lets the successor branch cache a discarded branch response as a trusted client artifact.",
			Prompt:                  SocketResponsePoisoningPrompt,
			ExpectedFiles:           []string{TargetSocketResponsePoisoningArtifact, TargetTrustedClientCacheArtifact, LanggraphForkArtifact},
			CheckpointSelector:      "before-unix-listener-launch",
			ForkVerificationMessage: SocketResponsePoisoningVerificationPrompt,
		},
		{
