...
func TestRunPartialFilesystemRollbackOpenFDPrimitiveConfirmsDeletedFDResidue(t *testing.T) {
	tmp := t.TempDir()
	result, err := Run(context.Background(), core.RunOptions{
	...
	after := mustReadJSON[core.Snapshot](t, filepath.Join(result.ArtifactDir, "snapshot-after.json"))
	...
	processAfter := mustReadJSON[core.ProcessSnapshot](t, filepath.Join(result.ArtifactDir, "process-after.json"))
...
func containsDeletedTrackedFD(snapshot core.ProcessSnapshot, trackedPath string) bool {
