...
func TestOrphanProcessOracleConfirmsLateEffect(t *testing.T) {
	before := core.Snapshot{Files: []core.FileEntry{}}
	after := core.Snapshot{Files: []core.FileEntry{{Path: "late-effect", Type: "file"}}}
...
func TestOrphanProcessOracleIgnoresExistingLateEffect(t *testing.T) {
	before := core.Snapshot{Files: []core.FileEntry{{Path: "late-effect", Type: "file"}}}
	after := core.Snapshot{Files: []core.FileEntry{{Path: "late-effect", Type: "file"}}}
...
func TestActionReplayOracleConfirmsDuplicateExternalEffect(t *testing.T) {
	var before effect.ExternalState
	var after effect.ExternalState
	after.Effects.Resources = []effect.EffectResource{
...
func TestAuthorityResurrectionOracleConfirmsStaleTokenReuse(t *testing.T) {
	var after effect.ExternalState
	after.Authority.Tokens = []effect.AuthorityToken{
		{Token: "tok_1", Consumed: true, ConsumedBy: "deploy-branch-a"},
	}
	replay := &effect.ConsumeTokenResponse{Error: "token_already_consumed", Accepted: false}
...
func TestPersistentShellPoisoningOracleConfirmsResidue(t *testing.T) {
	before := core.ShellState{PWD: "/workspace", GitResolution: "/usr/bin/git"}
	after := core.ShellState{
...
func TestPersistentShellPoisoningOracleIgnoresCleanShell(t *testing.T) {
	before := core.ShellState{PWD: "/workspace", GitResolution: "/usr/bin/git"}
	after := core.ShellState{PWD: "/workspace", GitResolution: "/usr/bin/git"}
...
func TestPartialFilesystemRollbackOracleConfirmsResidue(t *testing.T) {
	...
	after := core.Snapshot{Files: []core.FileEntry{
...
func TestPartialFilesystemRollbackFDOracleConfirmsDeletedFDResidue(t *testing.T) {
	after := core.Snapshot{Root: "/tmp/workspace", Files: []core.FileEntry{
		{Path: "tracked.txt", Type: "file", Mode: "-rw-r--r--"},
	}}
	processAfter := core.ProcessSnapshot{
...
func TestPartialFilesystemRollbackFDOracleMatchesRelativeWorkspace(t *testing.T) {
	...
	after := core.Snapshot{Root: "runs/example/workspace", Files: []core.FileEntry{
		{Path: "tracked.txt", Type: "file", Mode: "-rw-r--r--"},
	}}
	processAfter := core.ProcessSnapshot{
		Processes: []core.ProcessEntry{
