...
func runPersistentShellPoisoning(ctx context.Context, opts core.RunOptions) (*core.RunResult, error) {
	...
	filesBefore, err := core.SnapshotFilesystem(run.Workspace)
	if err != nil {
		return nil, err
	}
	if err := core.WriteJSON(filepath.Join(run.RunDir, "snapshot-before.json"), filesBefore); err != nil {
		return nil, err
	}
	...
	if err := core.WriteJSON(filepath.Join(run.RunDir, "shell-before.json"), before); err != nil {
		return nil, err
	}
	...
	if err := run.Trace.Write(core.NewEvent(run, "P4", "shell_state_mutated", map[string]any{
		"mutations": []string{"PATH", "cwd", "alias"},
	})); err != nil {
		return nil, err
	}
	processAfterMutation, err := core.RecordProcessSnapshot(ctx, env, run, "P4", "process-after-mutation.json")
	if err != nil {
		return nil, err
	}
	filesAfterMutation, err := core.SnapshotFilesystem(run.Workspace)
	if err != nil {
		return nil, err
	}
	if err := core.WriteJSON(filepath.Join(run.RunDir, "snapshot-after-mutation.json"), filesAfterMutation); err != nil {
		return nil, err
	}

	if isControlRun(opts) {
		if err := run.Trace.Write(core.NewEvent(run, "P6", "control_shell_restarted", map[string]any{
	...
	if err := core.WaitForTimingBoundary(ctx, run, "P8", "before_replay_probe"); err != nil {
		return nil, err
	}
	...
	if err := core.WriteJSON(filepath.Join(run.RunDir, "shell-after.json"), after); err != nil {
		return nil, err
	}
	processAfterReplay, err := core.RecordProcessSnapshot(ctx, env, run, "P8", "process-after-replay.json")
	if err != nil {
		return nil, err
	}
	if _, err := core.RecordProcessLineage(run, "P8", "process-lineage.json", processBefore, processAfterMutation, processAfterReplay, "process-before.json", "process-after-mutation.json", "process-after-replay.json"); err != nil {
		return nil, err
	}
	filesAfterReplay, err := core.SnapshotFilesystem(run.Workspace)
	if err != nil {
		return nil, err
	}
	if err := core.WriteJSON(filepath.Join(run.RunDir, "snapshot-after-replay.json"), filesAfterReplay); err != nil {
		return nil, err
	}
