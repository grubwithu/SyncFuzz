...
func targetRun(args []string) {
	...
	observationPlan := fs.String("observation-plan", "", "optional query-specific observation-plan.json")
	observationMode := fs.String("observation-mode", "", "observation-plan collection mode: shadow (default) or pruned-filesystem")
	envKind := fs.String("env", "local", "execution environment backend")
	containerImage := fs.String("container-image", "ubuntu:latest", "container backend image")
	if err := fs.Parse(args); err != nil {
		os.Exit(2)
