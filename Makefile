...
synthesis-langgraph-statefuzz-report: ebpf-build
	...
	$(SYNCFUZZ) synthesis statefuzz-relation-batch-report --objective "$(LANGGRAPH_SYNTHESIS_OBJECTIVE)" --root "$(LANGGRAPH_STATEFUZZ_BATCH_ROOT)" --out "$(LANGGRAPH_STATEFUZZ_BATCH_ROOT)/statefuzz-relation-batch-report.json"

synthesis-langgraph-statefuzz-relation-report: ebpf-build
	@test -n "$(LANGGRAPH_SYNTHESIS_OBJECTIVE)" && test -n "$(LANGGRAPH_STATEFUZZ_BATCH_ROOT)" || (echo "usage: make synthesis-langgraph-statefuzz-relation-report LANGGRAPH_SYNTHESIS_OBJECTIVE=<objective.json> [LANGGRAPH_STATEFUZZ_BATCH_ROOT=runs/langgraph-statefuzz]"; exit 2)
	$(SYNCFUZZ) synthesis statefuzz-relation-batch-report --objective "$(LANGGRAPH_SYNTHESIS_OBJECTIVE)" --root "$(LANGGRAPH_STATEFUZZ_BATCH_ROOT)" --out "$(LANGGRAPH_STATEFUZZ_BATCH_ROOT)/statefuzz-relation-batch-report.json"
...
synthesis-langgraph-v3-calibration: ebpf-build langgraph-profile-image
	...
	$(SYNCFUZZ) profile recovery-set --objective "$(LANGGRAPH_SYNTHESIS_OBJECTIVE)" --seed "$$root/state-seed.json" --passive-observation "$$passive_observation" --continuation-query "$(LANGGRAPH_V3_CONTINUATION_QUERY)" --out "$$root/historical-recovery-set.json"; \
...
synthesis-langgraph-v3-fidelity: ebpf-build langgraph-profile-image
	...
	for probe_mode in full pruned; do \
		...
		mkdir -p "$$mode_root"; \
		$(SYNCFUZZ) synthesis prepare-langgraph-fork --objective "$(LANGGRAPH_SYNTHESIS_OBJECTIVE)" --candidate "$(LANGGRAPH_SYNTHESIS_CANDIDATE)" --profile-run "$$root/profile-run.json" --binding "$$root/langgraph-native-frontier-binding.json" --model "$$model" --container-image "$(LANGGRAPH_PROFILE_IMAGE)" --runtime-root "$$mode_root/recovery-runtimes" --retained-resource-kind unix-socket-listener --retained-resource-path "$$socket" --passive-probe-mode "$$probe_mode" --continuation-query "$(LANGGRAPH_V3_CONTINUATION_QUERY)" --out-plan "$$mode_root/langgraph-fork-plan.json" --out-profile-run "$$mode_root/bound-profile-run.json" --out-workspace-topology "$$mode_root/workspace-topology.json"; \
		...
		$(SYNCFUZZ) profile recovery-set --objective "$(LANGGRAPH_SYNTHESIS_OBJECTIVE)" --seed "$$mode_root/state-seed.json" --passive-observation "unix-socket-listener-holder-v1:$$socket" --continuation-query "$(LANGGRAPH_V3_CONTINUATION_QUERY)" --out "$$mode_root/historical-recovery-set.json"; \
