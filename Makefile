...
MAF_PYTHON_ENV := $(if $(MAF_PYTHON),SYNCFUZZ_MAF_PYTHON='$(subst ','"'"',$(MAF_PYTHON))',)
MAF_MODEL_ENV := $(if $(MAF_MODEL),SYNCFUZZ_MAF_MODEL='$(subst ','"'"',$(MAF_MODEL))',)
MAF_COPILOT_CLI_ENV := $(if $(MAF_COPILOT_CLI),SYNCFUZZ_MAF_COPILOT_CLI='$(subst ','"'"',$(MAF_COPILOT_CLI))',)
MAF_SESSION_HOME_ENV := $(if $(MAF_SESSION_HOME),SYNCFUZZ_MAF_SESSION_HOME='$(subst ','"'"',$(MAF_SESSION_HOME))',)
MAF_LOG_LEVEL_ENV := $(if $(MAF_LOG_LEVEL),SYNCFUZZ_MAF_LOG_LEVEL='$(subst ','"'"',$(MAF_LOG_LEVEL))',)
MAF_ALLOW_UNSUPPORTED_ENV := $(if $(filter true,$(MAF_ALLOW_UNSUPPORTED_TASKS)),SYNCFUZZ_MAF_ALLOW_UNSUPPORTED_TASKS=true,)
...
target-maf-github-copilot-shell-check:
	@$(LOAD_DOTENV); target_python="$(MAF_PYTHON)"; test -n "$$target_python" || target_python="$$SYNCFUZZ_MAF_PYTHON"; test -n "$$target_python" || target_python="targets/maf_github_copilot_shell/venv/bin/python"; test -x "$$target_python" || target_python="python3"; $(MAF_MODEL_ENV) $(MAF_COPILOT_CLI_ENV) $(MAF_SESSION_HOME_ENV) $(MAF_LOG_LEVEL_ENV) "$$target_python" targets/maf_github_copilot_shell/run_target.py --check

target-maf-github-copilot-shell:
	$(LOAD_DOTENV); $(MAF_PYTHON_ENV) $(MAF_MODEL_ENV) $(MAF_COPILOT_CLI_ENV) $(MAF_SESSION_HOME_ENV) $(MAF_LOG_LEVEL_ENV) $(MAF_ALLOW_UNSUPPORTED_ENV) $(SYNCFUZZ) target run --target maf-github-copilot-shell --task $(TARGET_TASK) $(TARGET_PROMPT_PROFILE_ARGS) $(TARGET_RUN_ARGS) --command-file examples/target-commands/maf-github-copilot-shell.sh

target-maf-github-copilot-shell-suite:
	$(LOAD_DOTENV); $(MAF_PYTHON_ENV) $(MAF_MODEL_ENV) $(MAF_COPILOT_CLI_ENV) $(MAF_SESSION_HOME_ENV) $(MAF_LOG_LEVEL_ENV) $(MAF_ALLOW_UNSUPPORTED_ENV) $(SYNCFUZZ) target suite --target maf-github-copilot-shell --task $(TARGET_TASK) $(TARGET_TASKS_ARGS) $(TARGET_SEED_ARGS) $(TARGET_SEEDS_ARGS) $(TARGET_GROUP_ARGS) $(TARGET_GROUPS_ARGS) $(TARGET_PROMPT_PROFILE_ARGS) $(TARGET_PROMPT_PROFILES_ARGS) --repeat $(REPEAT) --corpus $(CORPUS) $(TARGET_RUN_ARGS) --command-file examples/target-commands/maf-github-copilot-shell.sh

target-maf-github-copilot-shell-matrix-suite:
	$(LOAD_DOTENV); $(MAF_PYTHON_ENV) $(MAF_MODEL_ENV) $(MAF_COPILOT_CLI_ENV) $(MAF_SESSION_HOME_ENV) $(MAF_LOG_LEVEL_ENV) $(MAF_ALLOW_UNSUPPORTED_ENV) $(SYNCFUZZ) target suite --target maf-github-copilot-shell --task $(TARGET_TASK) $(TARGET_TASKS_ARGS) $(TARGET_SEED_ARGS) $(TARGET_SEEDS_ARGS) $(TARGET_GROUP_ARGS) $(TARGET_GROUPS_ARGS) $(TARGET_PROMPT_PROFILE_ARGS) $(TARGET_PROMPT_PROFILES_ARGS) --matrix $(FEEDBACK_ARGS) $(CANDIDATE_LIMIT_ARGS) --repeat $(REPEAT) --corpus $(CORPUS) $(TARGET_RUN_ARGS) --command-file examples/target-commands/maf-github-copilot-shell.sh

target-maf-github-copilot-shell-campaign:
	$(LOAD_DOTENV); $(MAF_PYTHON_ENV) $(MAF_MODEL_ENV) $(MAF_COPILOT_CLI_ENV) $(MAF_SESSION_HOME_ENV) $(MAF_LOG_LEVEL_ENV) $(MAF_ALLOW_UNSUPPORTED_ENV) $(SYNCFUZZ) target campaign --target maf-github-copilot-shell --task $(TARGET_TASK) $(TARGET_TASKS_ARGS) $(TARGET_SEED_ARGS) $(TARGET_SEEDS_ARGS) $(TARGET_GROUP_ARGS) $(TARGET_GROUPS_ARGS) $(TARGET_PROMPT_PROFILE_ARGS) $(TARGET_PROMPT_PROFILES_ARGS) --rounds $(ROUNDS) --repeat $(REPEAT) $(FEEDBACK_ARGS) $(CANDIDATE_LIMIT_ARGS) --corpus $(CORPUS) --out $(OUT) $(TARGET_RUN_ARGS) --command-file examples/target-commands/maf-github-copilot-shell.sh
