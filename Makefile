...
.PHONY: help list fault-plans timing-profiles run-case run-pair run-mvp run-action run-authority run-shell run-fs run-branch run-suite run-diff-suite corpus-list corpus-show corpus-verify replay test-go fmt-go mock-build mock-start
...
help:
	...
	@echo "Variables: OUT=$(OUT), CORPUS=$(CORPUS), DELAY=$(DELAY), ENV=$(ENV), CONTAINER_IMAGE=$(CONTAINER_IMAGE), LIMIT=$(LIMIT), VERIFY_LIMIT=$(VERIFY_LIMIT), DIFFERENTIAL=$(DIFFERENTIAL), TIMING=$(TIMING), MOCK_URL=$(MOCK_URL)"
