...
Target matrix, suite, and campaign commands also accept `--candidate-scope all|seed-only|seed-plus-prompts` (or `TARGET_CANDIDATE_SCOPE=...` through Make). `seed-only` retains only caller-selected base seed scenarios; `seed-plus-prompts` additionally retains generated prompt variants but excludes semantic scenario derivations; `all` is the default full Scenario IR universe. The three scopes support a controlled seed / prompt-repair / semantic-mutator ablation without changing the selected seed family.

The current frozen Phase 5B snapshots are recorded in [PHASE5B_STRATEGY.md](docs/PHASE5B_STRATEGY.md): across three post-calibration policy blocks, feedback has the highest activation and coverage totals under the stated configuration; across two mutator-ablation blocks, the full semantic-mutator scope has not yet outperformed seed-only or seed-plus-prompts at the same 16-run budget. These are preliminary blocked repeats, not statistical or causal claims.
...
`target runtime-pair` closes the execution gap for a real controlled campaign:
it takes an explicitly named control command and target command for the same
adapter, target, and task, runs them in independent workspaces, then writes
`target-runtime-pair.json` plus the regular v2 pair differential. The caller
must implement the declared `fresh-runtime`, `branch-cleanup`, or
