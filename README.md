...
The Unix-listener motivation example now has a dedicated OS-level evidence
protocol in [docs/MOTIVATION_EXAMPLE_ROOT_CAUSE.md](docs/MOTIVATION_EXAMPLE_ROOT_CAUSE.md).
Its E0--E3 controls use pathname/FD probes, process identity, `SO_PEERCRED`,
and a post-fork capture witness to produce a local `root-cause.json`. This is
currently procfs and application evidence, not an eBPF collector or a generic
causal verdict.
