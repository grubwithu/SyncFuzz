...
The Scenario IR execution plan stored on a target matrix candidate is now also passed into the real target run. Its replay/fork choice, checkpoint selector, checkpoint backend, process mode, and fork follow-up become the runtime environment and are copied into `target-task.json`; candidate-specific late-observation timing is honored as well. Corpus replay and verify restore that stored plan instead of silently falling back to the built-in task plan. Matrix construction now uses that control surface for the first semantic phase-shift mutator: every built-in split-process checkpoint candidate gains a generated `phase-shift-single-process` sibling that runs the same task and oracle with initial/resumed phases kept in one runtime process. This is the first generated candidate that changes execution semantics rather than prompt wording. Primitive substitution, lifecycle splice across different operations, and cross-seed synthesis remain incomplete.
